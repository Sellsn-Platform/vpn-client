...
namespace SellSn.Client.OpenVPN;
...
public sealed class VpnManager : IVpnManager, IAsyncDisposable
{
    ...
    public Task ConnectAsync(string configurationPath, CancellationToken cancellationToken = default)
    {
        ...
        _managementSocketHandler =
            new ManagementSocketHandler(ManagementSocketHandler.GetAvailablePort(45555));

        // Set the process args (--dev-node {_configuration.TapAdapterName} removed)
        _ovpnProcess.StartInfo.Arguments =
            $"--config {configurationPath} --register-dns --management 127.0.0.1 {_managementSocketHandler.Port}";
